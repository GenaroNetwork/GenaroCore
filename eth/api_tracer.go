...
import (
	...
	"github.com/GenaroNetwork/Genaro-Core/eth/tracers"
	"github.com/GenaroNetwork/Genaro-Core/internal/ethapi"
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for i, tx := range txs {
		...
		if _, _, _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(msg.Gas()),&sentinelHeft); err != nil {
			failed = err
			break
		}
		// Finalize the state so any modifications are written to the trie
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
    ...
    var sentinelHeft uint64
	ret, gas, failed, err := core.ApplyMessage(vmenv, message, new(core.GasPool).AddGas(message.Gas()),&sentinelHeft)
	if err != nil {
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		var sentinelHeft uint64
		if _, _, _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(tx.Gas()),&sentinelHeft); err != nil {
