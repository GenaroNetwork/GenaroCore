...
import (
	...
	"github.com/GenaroNetwork/GenaroCore/consensus/clique"
	...
	"github.com/GenaroNetwork/GenaroCore/eth/filters"
	...
	"github.com/GenaroNetwork/GenaroCore/internal/ethapi"
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.Genaro != nil {
		return genaro.New(chainConfig.Genaro, db)
	}
...
func (s *Ethereum) StartMining(local bool) error {
	...
	if genaro, ok := s.engine.(*genaro.Genaro); ok {
		wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
		if wallet == nil || err != nil {
			log.Error("Etherbase account unavailable locally", "err", err)
			return fmt.Errorf("signer missing: %v", err)
		}
		genaro.Authorize(eb, wallet.SignHash)
	}
